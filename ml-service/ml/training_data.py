...
def get_full_dataset(seed=42):
    # Use real IBM corpus for normal behavior (20,000 queries)
    normal = generate_ibm_normal(seed=seed)
    
    # Use mutated IBM adversarial corpus for attack behavior (5,000 queries)
    anomalous = generate_ibm_adversarial(seed=seed)
...
def generate_ibm_adversarial(seed=42):
    """
    Load the adversarially mutated queries generated by our Node.js mutator.
    Run them through the feature extractor to produce a high-fidelity 'anomalous' training set.
    """
    import json
    import os
    try:
        ...
        print("[training_data] WARNING: Could not import feature extractor.")
        return generate_anomalous_samples(n=5000, seed=seed)

    corpus_path = os.path.join(os.path.dirname(__file__), '..', 'data', 'adversarial_corpus.json')
    if not os.path.exists(corpus_path):
        print(f"[training_data] WARNING: Adversarial corpus not found at {corpus_path}. Fallback to generic.")
        return generate_anomalous_samples(n=5000, seed=seed)

    with open(corpus_path, 'r') as f:
        queries = json.load(f)

    # Extract features from mutated queries
    raw = extract_features_batch(queries)
    corpus_df = pd.DataFrame(raw, columns=FEATURE_COLUMNS)
